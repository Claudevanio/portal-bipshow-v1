  ...
  return (
    <ContainerOrder>
      {user && loading ? (
        ...
        <>
          <p className="text-center text-xl pt-12 font-bold">{dataOrder?.mensagem}</p>
        </>
      ) : (
        <>
          <Box className="!flex !items-center !justify-center">
            ...
            dataOrder?.pedido?.status === 'AGUARDANDO' ? (
              <div className="payment flex flex-col items-center gap-4 justify-center p-8 w-full md:!w-1/3 ">
                ...
                <img src={dataOrder.pedido.pagamento.urlPagamento} alt="QR Code" />
                <p className="normal text-light">ou se preferir, você pode pagar copiando e colando o seguinte código</p>
                <div className="min-w-96 w-full">
                  <textarea rows={7} value={dataOrder.pedido.pagamento.textoPix} readOnly className="bg-grayLight w-full" id="chave-pix" />
                </div>
              ...
              <div className="status">
                <h4>
                  Seu pedido foi: <strong>{dataOrder?.pedido?.pagamento.status}</strong>
                </h4>
              </div>
            )}
          </Box>
      ...
      {!user && <p className="text-center text-xl pt-12 font-bold">Você precisa estar logado para acessar essa página.</p>}
    </ContainerOrder>
  );
};
