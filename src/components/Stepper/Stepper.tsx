...
export const Stepper: React.FC<IStepper> = ({ currentStep, steps, fontSize, hideNotCurrentMobile }) => {
  return (
    <ContainerStepper currentStep={currentStep} steps={steps}>
      {steps.map(step => (
        <Box
          key={`${step.stage}${step.number}`}
          className={`container-stepper ${step.number > 0 ? '' : 'disabled'} ${step.number <= currentStep.number ? 'active' : ''} ${step.number === currentStep.number ? 'actual' : ''}`}
          sx={
