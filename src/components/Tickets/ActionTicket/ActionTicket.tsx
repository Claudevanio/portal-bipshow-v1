...
export const Ticket: React.FC<{ width: number; height: number; color: string; fillOpacity?: boolean }> = ({ width, height, color, fillOpacity }) => (
      ...
      {eventTicket && eventTicket.tiposDeIngresso && eventTicket.tiposDeIngresso.length > 0 ? (
        <>
          <div className={`btn-submit ${ticket ? 'active' : ''}`}>
            ...
            <Button disabled={ticketsPurchase && ticketsPurchase.length > 0 ? false : true} onClick={handleShowModal}>
              Comprar
            </Button>
          </div>
