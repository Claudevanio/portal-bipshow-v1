...
export const CardTicket: React.FC<ICardTicket> = ({ valorVenda, index = 0, nome }) => {
  ...
  return (
    <ContainerCardTicket onClick={() => handleSelectTicket(index)}>
      {nome &&
        (nome?.split(' ').length <= 0 ? (
          <div className="date">
            <h6 className="name-extends">{nome}</h6>
          </div>
        ) : (
          <div className="date">
            <h6 className={`${nome.split(' ').length <= 2 && !nome.split(' ')[0].split('/')[1] ? 'two' : ''}`}>
              {nome.split(' ')[0].split('/')[0]}
              {nome.split(' ').length === 2 && !nome.split(' ')[0].split('/')[1] && ` ${nome.split(' ')[1]}`}
            </h6>
            {nome.split(' ')[0].split('/')[1] && (
              <div className="month">
                <span className="month">/{nome.split(' ')[0].split('/')[1]}</span>
                <span className="day-extend">{String(nome.split(' ')[1]).replace('(', '').replace(')', '')}</span>
              </div>
            )}
          </div>
        ))}
      <div className="price">
        {valorVenda && (
          <p className="text-dark">
            ...
            {valorVenda.length <= 1 &&
              `Preço de ${valorVenda[0].toLocaleString('pt-BR', {
                minimumFractionDigits: 2,
                style: 'currency',
                currency: 'BRL'
              })}`}
