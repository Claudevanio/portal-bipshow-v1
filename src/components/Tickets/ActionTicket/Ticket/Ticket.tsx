    ...
    <ContainerTicket>
              ...
              (data.nome.split(' ').length <= 0 ? (
                ...
                <>
                  <h6 className={`${data.nome.split(' ').length <= 2 && !data.nome.split(' ')[0].split('/')[1] ? 'two' : ''}`}>
                    ...
                    {data.nome.split(' ').length === 2 && !data.nome.split(' ')[0].split('/')[1] && ` ${data.nome.split(' ')[1]}`}
                  </h6>
                  <div className="month">
                    {data.nome.split(' ')[0].split('/')[1] && <span className="month">/{data.nome.split(' ')[0].split('/')[1]}</span>}
                    {data.nome.split(' ')[0].split('/')[1] && (
                      <span className="day-extend">{String(data.nome.split(' ')[1]).replace('(', '').replace(')', '')}</span>
                    )}
                  </div>
                </>
              ))}
      ...
      {data.tiposDeIngresso.map((item, index) => (
        <Action
          key={item.nome}
          nome={item.nome}
          limitePorUsuario={item.limitePorUsuario}
          totalDisponivel={item.totalDisponivel}
          qtd={item.qtde}
          taxaFixa={item.taxaFixa}
          taxaServico={item.taxaServico}
          ...
          exibirTaxaSomada={item.exibirTaxaSomada}
        />
      ))}
    </ContainerTicket>
  );
