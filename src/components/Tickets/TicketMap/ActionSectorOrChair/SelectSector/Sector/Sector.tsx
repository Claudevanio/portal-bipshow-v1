...
export const Sector: React.FC<ISector> = ({ nome, preco, setorId, valoresPorFormaPagamento, taxaPadrao }) => {
  const { handleSelectSector, selectedChairs } = useEventTicket();

  return (
    <ContainerSector
      type="button"
      onClick={() => handleSelectSector(Number(setorId), String(nome))}
      className={`${selectedChairs.find(item => item.idSector === setorId) ? 'active' : ''}`}
    >
      <div className="info">
        ...
        {valoresPorFormaPagamento &&
          Object.entries(valoresPorFormaPagamento).map(item => {
