...
export const Radio: React.FC<IRadio> = ({ id, name, label, readonly, defaultValue, remainColor = false, ...rest }) => {
    ...
    <Controller
      ...
      render={({ field: { name, onChange } }) => (
        <ContainerRadio className={`form-check ${remainColor ? 'remain-color' : ''}`}>
          ...
          />
          <label
            className={`form-check-label ${remainColor ? 'remain-color' : ''}`}
            htmlFor={`${id}-${rest.value}`}
            style={{
              paddingTop: 0
            }}
          >
