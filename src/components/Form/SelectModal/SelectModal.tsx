...
export const SelectModal: React.FC<SelectModalProps> = ({ options, titleModal, defaultOption, ...rest }) => {
  ...
  return (
    <ContainerSelectModal>
      ...
      <button type="button" onClick={handleOpenModal} className="btn-open-modal" disabled={rest.disabled}>
        {null}
      </button>
      <ContainerInputModalSelect>
        ...
        <div className="select">{isSelectOption.innerText}</div>
        {rest.loading && (
          <div className="spinner-border" role="status">
            <span className="visually-hidden">Loading...</span>
          </div>
        )}
        {rest.errorText && rest.errorText !== '' && <p className="text-dark">{rest.errorText}</p>}
      </ContainerInputModalSelect>
    </ContainerSelectModal>
  );
};
