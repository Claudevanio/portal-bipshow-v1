...
export const InputCode: React.FC<InputCodeProps> = ({ setCodigoValido, dispositivo, getCode }) => {
  ...
  useEffect(() => {
    ...
    if (combinedDigits.toString() == codigo) {
      ...
      if (combinedDigits.toString().length == 4) setShowWrongCode(true);
      else setShowWrongCode(false);
    ...
    <InputCodeCointainer>
      <div className="body">
          ...
          {digits?.map((digit: string, index: any) => (
            <input
              ...
              style={{
                ...
                margin: '0 4px',
                border: 'none',
                borderBottom: `2px solid ${digit ? '#8779F8' : 'rgba(0, 0, 0, 0.38)'}`,
        ...
        <div className="body-countdown">
          {showTimeLeft && <p className="countdown-disabled">Enviar um novo código</p>}
          {!showTimeLeft && <p onClick={handleSendNewCode}>Enviar um novo código</p>}

          {showTimeLeft && <p className="countdown"> | {timeLeft}</p>}
        </div>
        <div className="relative h-4">
          {showWrongCode && (
            <div className="text-xs mt-4 text-danger">
