...
export const Coupon: React.FC = () => {
  ...
  const { handleSubmitCouponDiscount, loadingCouponDiscount, couponAppliep, handleRemoveCoupon, amount, TICKET_PURCHASE_FROM_API } = useTicketPurchase();
            ...
            >
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>Valor Unitário (Taxa incluso)</TableCell>
                  <TableCell>Total</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>  
                {TICKET_PURCHASE_FROM_API && TICKET_PURCHASE_FROM_API?.ingressos && TICKET_PURCHASE_FROM_API.ingressos.map((item, index) => (
                  <TableRow key={index}>
                ...
                {couponAppliep && (
                  <TableRow>
                    <TableCell colSpan={1}>Desconto do cupom</TableCell>
                    <TableCell colSpan={2} />
                    <TableCell>
                      -{' '}
                ...
                {/* {amount &&
                  amount !== ticketsPurchase.reduce((acc, item) => acc + item.valor * item.qtde, 0) - (couponAppliep?.valorDesconto ?? 0) && (
                    <TableRow
                      style={{
                        backgroundColor: '#fcfcfc'
                      }}
                    >
                      <TableCell colSpan={1}>Taxa:</TableCell>
                      <TableCell colSpan={2} />
                      <TableCell>
                        {(
                          amount -
                          ticketsPurchase.reduce((acc, item) => acc + item.valor * item.qtde, 0) +
                          (couponAppliep?.valorDesconto ?? 0)
                      ...
                      {!couponAppliep && isCouponModalOpen && (
                        <form>
                          <FormProvider {...methods}>
                            ...
                            <div className="btn">
                              {!loadingCouponDiscount && <Button type="button" variant="outline-text" text="Aplicar" onClick={onSubmit} />}
                              {loadingCouponDiscount && <CircularProgress />}
                            </div>
                          </FormProvider>
                        </form>
                      )}
