...
export const Modal: React.FC<IModal> = ({ onClose, index, idTipo, open = true }) => {
  ...
  const isError = useMemo((): boolean => {
    if (!isForeign) return Object.keys(methods.formState.errors).length > 0;
  ...
  const onSubmit = async (data: IUser) => {
    ...
    if (
      ...
      data.cpf &&
      data.cpf.length > 4 &&
      (ticketSelectedUser.find(item => item.cpf === data.cpf) || ticketSelectedUser.find(item => item.nome === data.nome))
    ) {
  ...
  const isText = useMemo(() => {
    return <p className="text-[#000]">Os dados precisam ser reais para o acesso biométrico funcionar corretamente.</p>;
  }, []);
    ...
    if (isCpf && isCpf.length >= 14) {
      ...
      if (formattedCpf === user?.cpf) {
        callErrorDialogComponent(
          'O CPF digitado já é o titular desse ingresso. Informe o CPF para quem deseja transferir o ingresso.',
          TypeEnum.INFO
        );
