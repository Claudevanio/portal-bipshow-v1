...
export const Header: React.FC<IHeader> = ({ handleClose }) => {
  ...
  const { setIsStepper, setIsPurchaseSuccess, webView, purchaseSuccess } = useTicketPurchase();

  return (
    <ContainerHeader className={webView ? 'webview' : ''}>
      <div className="flex justify-center  items-center h-full px-6 md:p-0">
        <div
          className="flex justify-between bg-white p-6 h-full gap-5 items-center relative w-full md:w-1/2"
          style={{
            boxShadow: '0px 4px 16px rgba(0, 0, 0, 0.08)'
          }}
        >
          <Image
            image={`${eventTicket?.imagens?.minicapa?.link && eventTicket?.imagens?.minicapa?.link[0] === '/' ? baseUrlImages + eventTicket?.imagens?.minicapa?.link : eventTicket?.imagens?.minicapa?.link}`}
            className=" w-1/2 md:w-40 h-20 "
          />
          <div className="flex flex-col gap-1 w-1/2 font-medium ">
            <p className="text-dark text-sm">{eventTicket && eventTicket.nome}</p>
            <p className="text-textPrimary text-xs">
              {eventTicket && eventTicket?.localidade} -{eventTicket && eventTicket?.nomeDoLugar}
            </p>
            <p className="text-textPrimary text-sm">{ticket && ticket.tiposDeIngresso[0]?.dataUtilizacaoTexto?.split(' ')[1]}</p>
            <p className="text-textPrimary text-xs">{ticket && dayjs(eventTicket?.dataRealizacao).format('DD [de] MMMM [de] YYYY')}</p>
          </div>
