...
export function Navbar() {
  ...
  return (
    <>
      <nav className="flex items-center justify-between w-full md:px-10 px-4 py-6 flex-col md:flex-row gap-8 border-b-2 border-gray">
        <div className="flex items-center justify-between w-full md:w-2/5">
          ...
          <div className="flex items-center gap-4 relative">
            ...
            <div>
              <p className={fontMontSerrat.className + 'text-sm text-textPrimary font-bold'}>Comprou,</p>
              <p className={fontMontSerrat.className + 'text-sm text-textPrimary font-bold'}>sorriu, curtiu.</p>
            </div>
          </div>
          <div
          ...
          >
            {user?.imagem ? (
              <LegacyAvatar image={user?.imagem} username={user?.nome} variant="small-two" onClick={() => setProfileModalOpen(true)} />
            ) : (
              <Avatar className="cursor-pointer" onClick={() => setProfileModalOpen(true)} />
            )}
          </div>
        </div>
        <div
        ...
        >
          <div className="flex items-center gap-2 relative">
            <Image src={'/Localization.svg'} alt="Logo" width={30} height={30} />
            <div>
              <p className="text-sm text-primary font-medium">Eventos próximos à</p>
              <div className="text-textPrimary font-medium w-48 flex items-center gap-0">
                <div className="text-textPrimary font-medium relative max-w-40 min-w-40 text-ellipsis flex items-center">
                  <p className="max-w-32 overflow-hidden text-ellipsis whitespace-nowrap inline-block h-fit">
                    {locationValue.city !== null && locationValue.city !== undefined && locationValue.city !== ''
                      ? locationValue.city
                      : 'Sua localização'}
                  </p>
                  , {locationValue.uf ? locationValue.uf : 'UF'}
                </div>
                {locationValue.city && locationValue.uf && !anchorEl ? (
                  <CancelOutlined
                  ...
                  <ArrowDropDown className="text-primary mb-1 ml-[-8px] cursor-pointer" onClick={e => setAnchorEl(e.currentTarget)} />
                )}
                ...
                >
                  ...
                  >
                    <div className="flex  items-center absolute z-[9999] justify-between p-2 md:p-4">
                      <div className="flex flex-col justify-between gap-2 md:p-4">
                        <p className="text-primary font-medium">UF</p>
                        <Autocomplete
                          disablePortal
                          // disable clearable
                          // clearIcon={null}
                          ...
                          sx={{
                            width: '7rem',
                            position: 'relative',
                            zIndex: 9999, // Adicione zIndex aqui
                      ...
                      <div className="flex flex-col gap-2 p-4">
                        <p className="text-primary font-medium whitespace-nowrap w-fit">Selecione sua cidade</p>
                        <Autocomplete
                          disablePortal
          ...
          <div className="w-full md:w-3/5">
            <Input
              ...
              resetButton
              value={value}
              lefticon={<Search className="cursor-pointer" onClick={() => handleSearchEvents(value)} />}
          ...
          <div className="hidden items-center w-fit gap-4 md:flex">
            {!user ? (
              <>
                <Button
              ...
              <>
                ...
                >
                  {user?.imagem ? (
                    <LegacyAvatar image={user?.imagem} username={user?.nome} variant="small-two" />
                  ) : (
      ...
      >
        <div className="flex items-center justify-center w-full h-full">
          {isAuthModalOpen && (
            <div className="bg-white md:bg-[#00000000] w-full h-full overflow-y-auto md:overflow-hidden md:h-fit md:w-fit p-4 md:rounded-lg relative">
              <LoginAndRegister
                type={authType}
      ...
      >
        <div className="flex items-center flex-col p-4 w-full h-full bg-background">
          <div className="flex items-center justify-between w-full relative md:w-2/5">
            <Image
              className="absolute left-0 top-0 z-[-1] opacity-10 rotate-[-2deg] md:max-w-[26rem] sm:max-w-full overflow-hidden"
              src={'/LogoBackground.svg'}
              alt="Logo"
          ...
          {user ? (
            <div className="flex flex-col w-full items-start mt-8 gap-4 relative">
              <div className="flex items-center gap-4 relative">
                <div className="flex flex-col items-center gap-4 relative">
                  {user?.imagem ? (
                    <LegacyAvatar image={user?.imagem} username={user?.nome} variant="medium" onClick={() => setProfileModalOpen(true)} />
                  ) : (
                    <Avatar className="cursor-pointer" onClick={() => setProfileModalOpen(true)} />
                  )}
                </div>
                <div className="flex flex-col gap-2">
                  <p className="text-textPrimary font-medium">Olá, {user?.nome?.split(' ')[0]}</p>
                </div>
              </div>
              ...
              <Tabs
                ...
                className="mt-2"
                value={searchParams.get('tab') === 'meus-ingressos' ? 0 : searchParams.get('tab') === 'orders' ? 2 : pathName === '/profile' ? 1 : 0}
                onChange={(event, tab) => {
                  if (tab === 0) {
                    router.push('/profile?tab=meus-ingressos');
                    setProfileModalOpen(false);
