...
export const StepTwo: React.FC = () => {
  ...
  const { validateEmail, setEmailValidado, handleValidationEmail } = useAuth();
      ...
      {validateEmail && (
        <div>
          <div className="email-body">
            Enviamos o código para o email <p className="text">{getEmail()}</p> Digite-o abaixo para validar seu email.
          </div>
          <InputCode setCodigoValido={setEmailValidado} getCode={handleValidationEmail} dispositivo={getValues('email')} />
        </div>
      )}
    </ContainerStepTwo>
  );
