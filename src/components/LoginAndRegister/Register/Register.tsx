  ...
  return (
    <ul className="flex gap-3 items-center">
      {new Array(stepCount).fill(0).map((_, index) => (
        <li key={index} className={`w-4 h-1 rounded-xl ${activeTab === index ? 'bg-primary' : 'bg-[#D9D9D9]'}`} />
      ))}
    </ul>
  );
};

export const Register: React.FC<IRegister> = ({ handleChangeType, onClickPurchase }) => {
  const {
    isStepper,
  ...
  const onSubmit = React.useCallback(
    ...
    [isStepper, validateEmail, photo, photoInvalida, isInvalidPicture, createdUser]
  );
  ...
  return (
    <ContainerRegister>
      ...
      <div className="w-full flex flex-col items-center justify-center">
        ...
        <div className="card">
          <div className="w-full justify-center flex">
            <MiniStep activeTab={isStepper} stepCount={StepperRegister.length} />
          </div>
          <form onSubmit={handleSubmit(onSubmit)}>
            <h6 className="title">{isStepper === 4 ? 'Foto do rosto' : isStepper === 1 ? 'Endereço' : 'Criar conta'}</h6>
            {isStepper === 0 && <StepOne />}
            ...
            {isStepper === 0 && (
              <div className="text-sm py-4">
                ...
                <div>
                  <Checkbox checked={isTermsAccepted} onChange={e => setIsTermsAccepted(e.target.checked)} />
            ...
            {isStepper === 4 && <StepFive handleChangeType={handleChangeType} />}
            {!toPhoto && !(isStepper === 4 && !photoAvatar) ? (
              <div className={`${isStepper >= 0 ? 'buttons' : 'is-stepper-one-btn-custom'}`}>
                ...
                {isStepper === 4 && !photoAvatar ? (
                  <Button text="Tirar foto" variant="medium" type="button" onClick={() => onToPhoto(true)} />
                ) : (
                  <React.Fragment>
                    <Button
                      type="submit"
                      variant="medium"
                      text={isStepper === 4 ? (photoInvalida === 400 ? 'Nova foto' : 'Finalizar') : 'Avançar'}
                      disabled={
                        formState.isSubmitting ||
                        isLoading ||
                        !gender ||
                        (isStepper === 0 && !isTermsAccepted) ||
                        (!emailValidado && isStepper === 2 && validateEmail) ||
                        (isStepper === 4 && !photoAvatar)
                      }
                      loading={isLoading}
                    />
      ...
      <Modal open={isTermosModalOpen} onClose={handleCloseTermosModal} className="w-full h-full flex items-center justify-center">
        <div className=" h-full w-full md:w-1/2 md:h-1/2  bg-background p-4">
          <ButtonBack onClick={handleCloseTermosModal} />
          <iframe src={`html/${currentFilePath}`} className="w-full h-[80%]" />
        </div>
      </Modal>
    </ContainerRegister>
  );
