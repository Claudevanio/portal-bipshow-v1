  ...
  return (
    ...
    >
      <div className="w-[100%]  max-w-96 relative flex flex-col overflow-hidden group">
        <div className="w-[100%] h-[100%] relative z-[2] rounded-xl overflow-hidden">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={event.imagens.minicapa.link[0] === '/' ? baseUrlImages + event.imagens.minicapa.link : `${event.imagens.minicapa.link}`}
        ...
        <div className=" w-[100%] h-[100%] bg-gradient-to-t flex justify-between items-center from-background to-transparent">
          ...
          <div className="flex flex-col items-center justify-center w-fit text-textPrimary font-medium">
            <h3 className="text-lg font-medium first-letter:uppercase">{dayjs(event.dataRealizacao).format('MMM')}</h3>
            <h4 className="text-3xl font-bold">{dayjs(event.dataRealizacao).format('DD')}</h4>
            <p className="text-sm font-medium text-primary">{dayjs(event.dataRealizacao).format('HH:mm')}</p>
          </div>
        </div>
        <div className="mt-1">
          <Button className={'pl-1 border-primary font-semibold '} variant={event.prioridade === 1 ? 'primary' : 'secondary'}>
            <Image
              className="w-8 h-8"
