...
export default function Home() {
  ...
  useEffect(() => {
    if (isCp) {
      ...
      const mappedEventos = isCp.eventos.map((evento: any) => {
        const seenLocalIdAndName = seenLocalIDsAndNames.find(
          seenLocalIdAndName => seenLocalIdAndName.localId === evento.localId && seenLocalIdAndName.nome === evento.nome.split(' ')[0]
        );
      ...
      if (isCp.categorias) setCategorias([...isCp.categorias]);
  ...
  return (
    <main className="flex min-h-screen flex-col items-center">
            ...
            {categorias.map((categoria, index, arr) => {
              return (
                <div key={index} className="flex flex-col px-6 py-2 mx-[-.8rem] relative items-center justify-center gap-2 ">
                  {index !== arr.length - 1 && <div className="bg-background absolute right-0 h-8 w-6 bottom-[40%] z-10" />}
                  {index % 2 === 0 ? (
                    <>
                      <Image className="absolute top-0 left-0 rotate-[260deg]" src={'/single-border.svg'} height={24} width={95} alt="borda" />
                      <Image className="absolute top-0 right-0 rotate-[11deg]" src={'/single-border.svg'} height={24} width={95} alt="borda" />
                    </>
                  ) : (
                    <>
                      <Image className="absolute bottom-0 left-0 rotate-[190deg]" src={'/single-border.svg'} height={24} width={95} alt="borda" />
                      <Image className="absolute bottom-0 right-0 rotate-[80deg]" src={'/single-border.svg'} height={24} width={95} alt="borda" />
                    </>
                  )}
                  <div
      ...
      <div className="w-full flex items-center justify-between px-4 md:px-8 pb-8 border-b-2 border-gray">
        <h2 className="md:text-2xl flex gap-2 items-center font-bold text-textPrimary">
          <Image src={'/Calendar.svg'} alt="Calendario" width={30} height={30} />
          Todos os eventos
          {categoryFilter && (
            <button
        ...
        <p className={fontMontSerrat.className + 'md:text-2xl text-sm flex gap-2 items-center font-bold text-textPrimary'}>
          Comprou, <br />
          sorriu, curtiu.
        </p>
      </div>
